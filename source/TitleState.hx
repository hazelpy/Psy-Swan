...
class TitleState extends MusicBeatState
{
	...
	override public function create():Void
	{
		...
		// INPUT BS
		onKeyPress("F", function(d:Dynamic) {
			FlxG.fullscreen = !FlxG.fullscreen;
		}, "Toggle Fullscreen", null, false, true, false);

		onKeyPress("ENTER", function(d:Dynamic) {
			if (!transitioning && skippedIntro) continueToMenu();
			else if (!skippedIntro) skipIntro();
		}, "Proceed to Main Menu", null, false, true, true);

		#if mobile
		onScreenTouched(function(d:Dynamic) {
			if (!transitioning && skippedIntro) continueToMenu();
			else if (!skippedIntro) skipIntro();
		}, "MOBILE: Continue to Main Menu", true);
		#end

		#if !switch
		onKeyPress("START", function(d:Dynamic) {
			if (!transitioning && skippedIntro) continueToMenu();
			else if (!skippedIntro) skipIntro();
		}, "Proceed to Main Menu", null, false, true, true, "gamepad");
		#else
		onKeyPress("B", function(d:Dynamic) {
			if (!transitioning && skippedIntro) continueToMenu();
			else if (!skippedIntro) skipIntro();
		}, "Proceed to Main Menu (Switch)", null, false, true, true, "gamepad");
		#end
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			{
				case 1:
					addMoreText('Psych Engine by', 45);
				...
				// credTextShit.screenCenter();
				case 5:
					addMoreText('This is a mod to', -60);
				case 7:
